...
_EXACT_ROUTES: dict[tuple[str, str], tuple[str, str]] = {
    ...
    ("POST", "/agent/ask"): ("market_agent_ask", "agent"),
    ("GET", "/agent/preferences"): ("market_agent_preferences", "agent"),
}
...
_PREFIX_ROUTES: tuple[tuple[str, str, str], ...] = (
    ...
    ("/v1/data", "market_data_export", "data"),
    ("/v1/prices", "market_data_prices", "data"),
    ("/export", "market_data_export", "data"),
    ("/index/resolve", "index_resolve", "index"),
    ("/index/lookup", "index_lookup", "index"),
    ("/index/stats", "index_stats", "index"),
...
def instrumented_tool_names() -> frozenset[str]:
    from .market_mcp_registry import list_tools

    names = {t["name"] for t in list_tools("default")}
    names.update({"index_resolve", "index_lookup", "index_stats", "market_agent_ask"})
...
class ObservatoryMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        retailer = request.query_params.get("store") or request.query_params.get("retailer")
        ...
        loop.run_in_executor(
            ...
            lambda: record_agent_event(
                ...
                country=(country or "")[:8] or None,
                organization_id=identity.get("organization_id"),
                latency_ms=latency_ms,
                retailer=(retailer or "")[:64] or None,
