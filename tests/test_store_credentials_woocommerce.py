...
def test_wc_consumer_key_secret_pair(monkeypatch):
    ...
    monkeypatch.setenv("STORE_XRAY_PE_WC_CONSUMER_KEY", "ck_test")
    store_credentials.reload_credentials()

    monkeypatch.setenv("STORE_XRAY_PE_WC_CONSUMER_SECRET", "cs_test")
    store_credentials.reload_credentials()
    cfg = store_credentials.resolve_store_config("xray_pe")
    assert cfg["wc_consumer_key"] == "ck_test"
    assert cfg["wc_consumer_secret"] == "cs_test"
    assert store_credentials.has_store_credentials("xray_pe")
